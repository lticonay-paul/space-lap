...
export default function Catalog() {
  ...
  useEffect(() => {
    fetch(`${import.meta.env.VITE_API_URL}/laptops`)
      .then(res => res.json())
      .then(data => { setLaptops(data); setLoading(false); })
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0a0a0a", color: "#fff" }}>
      ...
      <div style={{ textAlign: "center", padding: "2rem 1.2rem 1.5rem" }}>
        ...
        <p style={{ color: "#555", fontSize: "0.85rem", margin: "0 0 1.2rem" }}>Laptops seleccionadas para cada misión</p>
        <input
          type="text" placeholder="Buscar laptop o marca..."
      ...
      {/* Botón filtros — solo celular */}
      <div style={{ padding: "0 1.2rem 1rem", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <p style={{ color: "#444", fontSize: "0.82rem", margin: 0 }}>{filtered.length} laptop{filtered.length !== 1 ? "s" : ""} encontrada{filtered.length !== 1 ? "s" : ""}</p>
        <button
      ...
      {/* Filtros visibles en celular cuando se abre */}
      ...
      <div style={{ display: "flex", maxWidth: "1200px", margin: "0 auto", padding: "0 1.2rem 3rem", gap: "1.5rem", alignItems: "flex-start" }}>
        ...
        <main style={{ flex: 1, minWidth: 0 }}>
          {filtered.length === 0
            ? <div style={{ textAlign: "center", color: "#444", padding: "4rem" }}>No se encontraron laptops</div>
